...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const logoutBtn = document.querySelector(
    'a[href="#"].text-orange-600:last-of-type'
  );
  if (logoutBtn) {
...
function isTimeSlotAvailable(
  ...
  try {
    ...
    if (newStartMinutes < openingTime || newEndMinutes > closingTime) {
      throw new Error(
        "Reservations are only available between 9:00 AM and 12:00 AM"
      );
    }
...
function calculatePrice(serviceType, startTime, additionalMembers = []) {
  ...
  if (serviceType === "court") {
    ...
    const additionalMembersArray = Array.isArray(additionalMembers)
      ? additionalMembers
      : [];
...
// Dialog modules
async function showPricingDialog(
  serviceType,
  price,
  additionalMembers,
  startTime
                ...
                <div class="mt-3">
                    <table class="mx-auto">
                        ...
                        <tr class="border-t">
                            ...
                            <td class="pt-2 font-bold" id="totalAmount">₱${
                              initialParticipants * RATE_PER_PERSON
                            }</td>
                        </tr>
                    </table>
                </div>
...
async function showAdminLogin(e) {
  ...
  const { value: formValues } = await Swal.fire({
    ...
    customClass: {
      ...
      htmlContainer: "px-6",
      confirmButton:
        "w-full bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded px-4 py-2",
      cancelButton:
        "w-full bg-gray-300 hover:bg-gray-400 text-gray-700 font-semibold rounded px-4 py-2",
      actions: "grid grid-cols-2 gap-4 px-6 pb-6 pt-6",
      validationMessage:
        "bg-red-100 text-red-500 rounded py-2 px-4 mt-3 text-sm font-medium",
    },
