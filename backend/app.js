...
app.post("/api/supplements", async (req, res) => {
  try {
    ...
    if (!supplement_name || quantity === undefined || price === undefined) {
      return res
        .status(400)
        .json({ error: "Supplement name, quantity, and price are required" });
    }
...
app.post("/api/buy-supplement", async (req, res) => {
  try {
    ...
    if (!supplementId || !quantity) {
      return res
        .status(400)
        .json({ error: "Supplement ID and quantity are required" });
    }
