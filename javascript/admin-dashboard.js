...
function displayDueMembers(dueMembers) {
  ...
  function getRiskIndicator(member) {
    ...
    if (daysUntilDue <= 3) {
      return { class: "bg-red-100 text-red-800 border-red-200", text: "Urgent" };
    } else if (daysUntilDue <= 5) {
      return { class: "bg-yellow-100 text-yellow-800 border-yellow-200", text: "Soon" };
    }
    return { class: "bg-green-100 text-green-800 border-green-200", text: "Upcoming" };
  }
            ...
            <div class="flex justify-between items-center p-4">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-600">Due: ${new Date(member.end_date).toLocaleDateString()}</p>
                    </div>
                </div>
                <div class="flex items-center space-x-2">
                    <span class="px-2 py-1 text-xs font-medium rounded-full ${riskIndicator.class} border">
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const membershipPieChart = new Chart(pieCtx, {
    ...
    data: {
      ...
      datasets: [
        {
          data: [30, 40],
          backgroundColor: ["rgb(74, 222, 128)", "rgb(59, 130, 246)"],
          borderColor: "white",
          borderWidth: 2,
        },
      ],
  ...
  if (ctx) {
    ...
    setInterval(loadDueMembers, 5000);
    setInterval(updateMembershipPieChart, 5000);
    setInterval(updateMemberGrowthChart, 5000);
  }
});
